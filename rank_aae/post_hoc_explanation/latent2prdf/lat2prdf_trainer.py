...
from rank_aae.post_hoc_explanation.latent2prdf.lat2prdf_dataloader import get_latent2prdf_dataloaders
...
class Latent2PRDFTrainer:
    ...
    @classmethod
    def from_data(cls, pkl_fn, element, igpu=0, batch_size=512, lr=1.0E-3, max_epoch=300,  sch_factor=0.25, sch_patience=300, 
                  style_noise=0.01, weight_decay=1e-2, optimizer_name="AdamW", verbose=True, work_dir='.',
                  dropout_rate=0.05):
        dl_train, dl_val, _ = get_latent2prdf_dataloaders(pkl_fn, batch_size, element=element)
